...
exports.onHyperLiquidUpdate = onHyperLiquidUpdate;
exports.fetchHyperLiquidFees = fetchHyperLiquidFees;
exports.fetchHyperLiquidOrderbook = fetchHyperLiquidOrderbook;
exports.initializeHyperLiquidPrices = initializeHyperLiquidPrices;
exports.startHyperLiquidPriceUpdater = startHyperLiquidPriceUpdater;
exports.getHyperLiquidPrices = getHyperLiquidPrices;
exports.getSpotPrice = getSpotPrice;
exports.getPerpPrice = getPerpPrice;
let hlPrices = {
    spot: 0,
    perp: 0,
    timestamp: 0,
...
function onHyperLiquidUpdate(cb) {
    updateCallback = cb;
}
 ...
 */
async function fetchHyperLiquidFees() {
    try {
        const response = await fetch(`${HL_API_URL}/info`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
 ...
 */
async function fetchHyperLiquidOrderbook(market = 'HYPE') {
    const tryUrls = [
    ...
    for (const url of tryUrls) {
        try {
            ...
            if (Array.isArray(data.bids) && Array.isArray(data.asks)) {
                ...
                bids = data.bids.map((b) => Array.isArray(b) ? [Number(b[0]), Number(b[1])] : [Number(b.price), Number(b.size)]);
                asks = data.asks.map((a) => Array.isArray(a) ? [Number(a[0]), Number(a[1])] : [Number(a.price), Number(a.size)]);
                return { bids, asks };
            }
            ...
            if (Array.isArray(data)) {
                ...
                if (obj && obj.bids && obj.asks) {
                    bids = obj.bids.map((b) => [Number(b[0]), Number(b[1])]);
                    asks = obj.asks.map((a) => [Number(a[0]), Number(a[1])]);
                    return { bids, asks };
                }
            }
        }
 ...
 * 初始化 HyperLiquid 价格
 */
async function initializeHyperLiquidPrices() {
    try {
 ...
 */
function startHyperLiquidPriceUpdater() {
