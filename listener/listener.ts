...
import v3Abi from './v3_abi.json';
import v4Abi from './v4_abi.json';
import v4PoolAbi from './v4_pool_abi.json';

// Import HyperLiquid 数据源
import { initializeHyperLiquidPrices, startHyperLiquidPriceUpdater, getHyperLiquidPrices, onHyperLiquidUpdate, fetchHyperLiquidFees, fetchHyperLiquidOrderbook } from './hyperliquid';
import fs from 'fs';
...
// 私钥（生产环境请确保通过安全方式注入）
const PRIVATE_KEY = (process.env.PRIVATE_KEY ?? '0x...') as Hex;

// 合约地址
const HYPE_USDT_V3_POOL: Address = (process.env.HYPE_USDT_V3_POOL ?? '0x...') as Address;
const HYPE_USDT_V4_POOL: Address = (process.env.HYPE_USDT_V4_POOL ?? '0x...') as Address;
const USDT_ADDRESS: Address = (process.env.USDT_ADDRESS ?? '0x...') as Address;
const HYPE_ADDRESS: Address = (process.env.HYPE_ADDRESS ?? '0x...') as Address;
...
// 去重缓存：防止同一个事件被处理多次
const processedLogsCache = new Map<string, boolean>();
const DEDUP_TIMEOUT_MS = 5000; // 5秒内的重复日志被视为同一事件

/**
 * 检查日志是否已被处理过（去重逻辑）
 */
function isDuplicateLog(blockNumber: number | bigint, transactionHash: string, logIndex: number | bigint): boolean {
...
let v3CurrentPrice: PoolPrice | null = null;
let v4CurrentPrice: PoolPrice | null = null;
...
async function printAllPrices() {
    ...
    const hlPrices = getHyperLiquidPrices();
    if (hlPrices.spot > 0) {
    ...
    if (prices.length > 0) {
        ...
        if (firstLastDiffPercent > PRICE_DIFF_SAVE_THRESHOLD_PERCENT) {
            try {
                ...
                const outPath = path.join(dataDir, `price_alerts-${day}.csv`);
                ...
                if (!fs.existsSync(outPath)) {
                    fs.writeFileSync(outPath, header, { flag: 'w' });
                }
                ...
                try {
                    const obSpot = await fetchHyperLiquidOrderbook('HYPE');
                    const obPerp = await fetchHyperLiquidOrderbook('HYPE-PERP');

                    function computeCapacityFromOrderbook(ob: any, buyPrice: number, sellPrice: number) {
                        if (!ob) return { buyAtLow: 0, sellAtHigh: 0 };
                ...
                (async () => {
                    try {
                        ...
                        const lowAbi = lowSource === 'V3 池' ? (v3Abi as any) : (v4Abi as any);
                        const highAbi = highSource === 'V3 池' ? (v3Abi as any) : (v4Abi as any);
                        ...
                        const hlFees = await fetchHyperLiquidFees();
                        const DEFAULT_HL_FEE = 0.05; // 万5

                        // 根据来源决定买卖双方手续费
                        function feeForSource(source: string, isBuySide: boolean): number {
                            if (source === 'V3 池') return V3_FEE;
                            if (source === 'V4 池') return V4_FEE;
                            if (source === '现货价格') return (hlFees.spotFee !== null ? Number(hlFees.spotFee) : DEFAULT_HL_FEE);
                            if (source === '合约价格') return (hlFees.perpFee !== null ? Number(hlFees.perpFee) : DEFAULT_HL_FEE);
                        ...
                        if (m <= 0) {
                            ...
                            if (highSource === '现货价格') estimatedCapacity = Math.min(estimatedCapacity, spotCapNum);
                            if (highSource === '合约价格') estimatedCapacity = Math.min(estimatedCapacity, perpCapNum);
                        }
 ...
 */
async function estimateCapacityFromRecentSwaps(
    poolAddress: Address,
    poolAbi: any,
    currentPrice: number,
    ...
    try {
        const swapEventAbi = (poolAbi as any).find((item: any) => item.name === 'Swap' && item.type === 'event');
        if (!swapEventAbi) return 0;

        const currentBlock = await publicClient.getBlockNumber();
        const fromBlock = currentBlock > BigInt(lookbackBlocks) ? currentBlock - BigInt(lookbackBlocks) : 0n;

        const logs = await publicClient.getLogs({ address: poolAddress, event: swapEventAbi, fromBlock, toBlock: currentBlock }) as any[];
...
function calculateArbitrageOpportunity(
    ...
    cheapFee: number, // 手续费百分比
    expensiveFee: number, // 手续费百分比
    tradeAmount: number // USDT 金额
): { canProfit: boolean; profit: number; profitPercent: number; details: string } {
 ...
 */
async function initializePoolPrice(poolAddress: Address, poolName: string): Promise<PoolPrice | null> {
    try {
        ...
        // 使用正确的 ABI 对象
        const abi = poolName === 'V3' ? (v3Abi as any) : (v4Abi as any);
        
        // 找到 Swap 事件的 ABI 定义
        const swapEventAbi = abi.find((item: any) => item.name === 'Swap' && item.type === 'event');
...
async function startListener() {
        ...
        if (priceDiff >= ARBITRAGE_THRESHOLD_PERCENT) {
            ...
            if (v4CurrentPrice.actualPrice > v3CurrentPrice.actualPrice) {
                const arb = calculateArbitrageOpportunity(
                    v3CurrentPrice.actualPrice,
                    v4CurrentPrice.actualPrice,
                    V3_FEE,
                    V4_FEE,
                    SIMULATED_TRADE_AMOUNT_USDT
                );
    ...
    await initializeHyperLiquidPrices();
    // 同步尝试读取 HyperLiquid 手续费信息（如果可用）
    try {
        const fees = await fetchHyperLiquidFees();
    ...
    startV3Listener();
    startV4Listener();
    startHyperLiquidPriceUpdater();
    // 注册 HyperLiquid 更新回调，确保 HL 价格变动也触发统一打印
    try {
        onHyperLiquidUpdate(printAllPrices);
    } catch (e) {
        // 忽略注册错误
    }
